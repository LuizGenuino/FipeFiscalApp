...
export class ApiService {
    ...
    constructor(type: string, path: string) {
        this.path = path;
        this.baseUrl = type === 'Embarcada' ? API_BASE_URL_EMBARCADA : API_BASE_URL_BARRANCO;
    }
    ...
    async post<T>(data: any): Promise<AxiosResponse> {
        const formData = await toFilteredFormData(data);
        return this.request<T>('POST', '', formData);
    }

    private async request<T>(method: 'GET' | 'POST', params?: string, body?: FormData): Promise<AxiosResponse> {
        const token = await this.getToken();
        const url = `${this.baseUrl}${this.path}${params ? '/' + params : ''}`;
        ...
        try {
            ...
            if (method === 'GET') {
                ...
                console.log("🟡 FormData preparado:", body);

                return await this.uploadWithRNFetchBlob(url, body, token || null);
            }
